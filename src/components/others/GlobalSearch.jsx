...
//       });
//       setShowBadge(true);
//       if (payload.notification.title === "New Whatsapp Message") {
//         queryClient.invalidateQueries(["all-whatsapp-message-by-id"]);
//         queryClient.invalidateQueries(["all-whatsapp-conversation"]);
//       }
//     } else {
//       playNewNotificationSound();
//       toaster.create({
...
import { messaging } from "@/firebase"; // ✅ use shared instance
import { onMessage } from "firebase/messaging";
...
const GlobalSearch = () => {
  ...
  return (
    <>
      <div className="flex items-center pt-[20px] pe-[30px] bg-gray-100 justify-between lg:justify-end">
        <Button
          onClick={() => setDrawerOpen(true)}
          className="lg:hidden ms-[20px] border-2 p-2 rounded-md"
        >
          <RenderIcon iconName="MenuIcon" size={28} />
        </Button>

        <div className="flex items-center gap-[20px]">
          <Avatar.Root className="bg-gray-100" variant="subtle">
            <Avatar.Fallback onClick={handleNotificationLog}>
              <RenderIcon iconName="NotificationIcon" size={18} />
            </Avatar.Fallback>
          ...
          <span onClick={() => setShowModal(!showModal)}>
            <RenderIcon iconName="LogoutIcon" size={24} />
          </span>
