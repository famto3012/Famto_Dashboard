...
const EditService = ({ isOpen, onClose, serviceId }) => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => fetchSingleService(serviceId, navigate),
    enabled: isOpen,
  });
  ...
  const handleEditService = useMutation({
    ...
    mutationFn: (data) => updateServiceDetail(serviceId, data, navigate),
    onSuccess: () => {
    ...
    onError: (data) => {
      toaster.create({
        title: "Error",
        description: data?.message || "Error while updating service detail",
        type: "error",
      });
    },
  });
  ...
  const handleSave = () => {
    ...
    handleEditService.mutate(formDataObject);
  };
          ...
          {showLoading ? (
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex items-start">
                ...
                <div className="flex items-center justify-start gap-[30px] w-2/3">
                  <figure className="h-16 w-16 rounded-md">
                    <img
                      src={previewURL || formData.bannerImage}
                      alt={formData.title}
              ...
              <div className="flex items-center justify-end gap-4 mt-[30px]">
                ...
                <button
                  disabled={handleEditService.isPending}
                  onClick={handleSave}
                  className="bg-teal-800 rounded-lg px-6 py-2 text-white font-semibold justify-end"
                >
                  {handleEditService.isPending ? "Saving..." : "Save"}
                </button>
              </div>
            </div>
          )}
