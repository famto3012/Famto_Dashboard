...
import { fetchAgentAppTimings } from "@/hooks/agent/useAgent";
import { createMerchantAgent } from "@/hooks/agent/useAgent";
import CropImage from "@/components/others/CropImage";
import { Button } from "@/components/ui/button";

const MerchantAddAgent = ({ isOpen, onClose }) => {
  ...
  const handleAddAgent = useMutation({
    mutationKey: ["add-merchant-agent"],
    mutationFn: (data) => createMerchantAgent(data, navigate),
    onSuccess: () => {
  ...
  const isLoading = geofenceLoading || appTimingsLoading;
  const isError = geofenceError || appTimingsError;
        ...
        <DialogBody>
          {isLoading ? (
            ...
            <div className="flex flex-col gap-4 max-h-[30rem] overflow-y-auto">
              ...
              <div className="flex items-center mt-5 gap-4">
                <label className="w-1/2 text-gray-500" htmlFor="workTimings">
...
export default MerchantAddAgent;
